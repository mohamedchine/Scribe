...
const createCommentCtrl = asyncHandler(async(req,res) =>{
    ...
    const comment  =await commentMdl.create({
...
const deleteCommentCtrl =asyncHandler( async(req,res)=>{
    ...
    if( !req.user.isAdmin && !req.user._id.equals(comment.Authorid)){
...
const updateCommentCtrl =asyncHandler( async(req,res)=>{
    ...
    if(!comment.Authorid.equals(req.user.id)) return res.status(403).json({message : "only comments author can update it"});
